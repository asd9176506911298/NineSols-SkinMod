...
namespace MG.GIF {
    ...
    public class Decoder : IDisposable {
        ...
        Image ReadImageBlock() {
            ...
            // return image
            if (TransparentIndex != NoTransparency) {
                var transAlpha = new Color32(0, 0, 0, 0);
                var tc = ActiveColourTable[TransparentIndex];
                for (int i = 0; i < Output.Length; ++i) {
                    if (Output[i].r == tc.r && Output[i].g == tc.b && Output[i].b == tc.b) {
                        Output[i] = transAlpha;
                    }
                }
            }
